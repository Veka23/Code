...
extern "C" void __iar_program_start( void ); // opisivaet prototip fynkcii _iar_program_start
...
public:
 ...
 {
    usartDriver.OnNextByteTransmit(); //vyzov metoda OnNextByteTransmt
 }
};
...
extern "C" const tIntVectItem __vector_table[] =
{
  ...
  0,
  xPortPendSVHandler,
  OsWrapper::Rtos::HandleSysTickInterrupt,//preryvanie po sistemnomy timery
  //External Interrupts
...
void DummyModule::handler()   { for(;;) {} } ; // obrabotchik preryvaniya
...
void __iar_program_start( void ) //realizazia iar_program
{
  __iar_init_core();
  __iar_init_vfp();
  __cmain();
